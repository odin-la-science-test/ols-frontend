...
Pour les elements **flottants/overlays uniquement** (jamais pour le layout). Ne jamais ecrire `bg-{token}/XX backdrop-blur-*` en inline — utiliser le tier correspondant.
...
**Regle** : `surface-low`/`surface-high` pour le layout, `glass-*` pour les overlays. Ne jamais utiliser `glass-surface`/`glass-chrome` sur des elements de layout (deprecated).
