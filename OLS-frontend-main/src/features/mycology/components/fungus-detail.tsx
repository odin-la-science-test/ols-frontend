...
export function FungusDetail({ fungus }: FungusDetailProps) {
      ...
      <div className="space-y-6">
        ...
        {(fungus.optimalTemperature || fungus.maximalTemperature || fungus.cultureMedium) && (
          <DetailSection title={t('scientific.growthConditions')} icon={Thermometer}>
            {fungus.optimalTemperature && (
              <DetailRow label={t('scientific.optimalTemperature')} value={`${fungus.optimalTemperature}°C`} />
            )}
            {fungus.maximalTemperature && (
              <DetailRow label={t('scientific.maximalTemperature')} value={`${fungus.maximalTemperature}°C`} />
            )}
        ...
        {(fungus.metabolism || fungus.secondaryMetabolites?.length || fungus.enzymes?.length || fungus.degradableSubstrates?.length) && (
          <DetailSection title={t('scientific.biochemicalProfile')} icon={Dna}>
            ...
            <DetailTags label={t('scientific.secondaryMetabolites')} tags={fungus.secondaryMetabolites || []} variant="molecule" />
            <DetailTags label={t('scientific.enzymes')} tags={fungus.enzymes || []} variant="success" />
            <DetailTags label={t('scientific.degradableSubstrates')} tags={fungus.degradableSubstrates || []} variant="default" />
          </DetailSection>
        )}
        ...
        <DetailSection title={t('scientific.clinicalInfo')} icon={AlertTriangle}>
          ...
          <DetailTags label={t('scientific.hosts')} tags={fungus.hosts || []} variant="warning" />
