...
import { useDeleteNote, useRestoreNote, useTogglePin } from '../hooks';
...
export function NoteList({ notes, isLoading, onSelectNote, selectedNoteId }: NoteListProps) {
  ...
  if (isLoading) {
    return <ListSkeleton layout="grid" count={6} itemHeight="h-36" columns="grid-cols-1 sm:grid-cols-2 lg:grid-cols-3" />;
  }

  if (notes.length === 0) {
    return <EmptyState icon={Tag} title={t('notes.emptyTitle')} description={t('notes.emptyDesc')} />;
  }
  ...
  const handleDelete = (e: MouseEvent) => {
    ...
    deleteMutation.mutate(note.id, {
      onSuccess: () => {
        ...
        useHistoryStore.getState().pushCommand('notes', {
      ...
      <div className="flex items-start gap-2 mb-1.5">
        ...
        <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
          <Tooltip delayDuration={200}>
            ...
            <TooltipContent side="top">{t('notes.pin')}</TooltipContent>
          </Tooltip>
          <Tooltip delayDuration={200}>
            <TooltipTrigger asChild>
              <button
            ...
            <TooltipContent side="top">{t('notes.delete')}</TooltipContent>
          </Tooltip>
        </div>
      </div>
