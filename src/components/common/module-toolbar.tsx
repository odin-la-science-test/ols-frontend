...
//
// In classic mode (no menu bar), 'menu' actions appear in an overflow dropdown
// (⋯ button) so they remain accessible without the menu bar.
...
export function ModuleToolbar({ className }: ModuleToolbarProps) {
  ...
  // Direct toolbar actions (always shown as buttons)
  const toolbarActions = toolbar?.actions.filter(
    (a) => a.placement === 'toolbar' || a.placement === 'both'
  ) ?? [];

  // In classic mode, 'menu' actions go into an overflow dropdown (no menu bar available)
  const overflowActions = isClassic
    ? toolbar?.actions.filter((a) => a.placement === 'menu') ?? []
    : [];
    ...
    <div className={cn('flex items-center gap-0.5 shrink-0', className)}>
      ...
      {toolbarActions.map((action) => {
        ...
        return (
          ...
          >
            <DynamicIcon name={action.icon} className="h-3.5 w-3.5 shrink-0" />
            {t(action.labelKey)}
          </Button>
        );
      })}
      ...
      {/* Overflow menu for 'menu' actions in classic mode (rightmost position) */}
