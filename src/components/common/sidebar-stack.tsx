...
const SidebarStackContext = createContext<SidebarStackContextValue | null>(null);
...
export function useSidebarStackContext(): SidebarStackContextValue | null {
  return useContext(SidebarStackContext);
}
//
//
...
export function StackLayoutToggle({ side, className }: StackLayoutToggleProps) {
  ...
  return (
    <Tooltip delayDuration={200}>
      ...
      <TooltipContent side="bottom">{layout === 'tabs' ? 'Vue empilée' : 'Vue onglets'}</TooltipContent>
    </Tooltip>
  );
}
