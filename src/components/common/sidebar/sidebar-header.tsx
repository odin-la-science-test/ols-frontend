...
import { useOverlayDragControls } from './overlay-drag-context';
...
export function SidebarHeader({
  ...
  // Drag libre en mode pinned (framer-motion)
  const overlayDrag = useOverlayDragControls();

  // Sidebar-zone draggable (primary/secondary, sauf en mode pinned)
  const { attributes: zoneDragAttrs, listeners: zoneDragListeners, setNodeRef: setZoneDragRef } = useDraggable({
    ...
    disabled: isActivity || isPinned,
  });

  // Le titre est draggable soit pour le zone-swap (dnd-kit) soit pour le drag libre (pinned)
  const isDraggable = !isActivity || isPinned;
  const useZoneDrag = !isActivity && !isPinned;
  ...
  return (
    ...
    >
      {/* Title — drag handle pour zone-swap (primary/secondary) ou drag libre (pinned) */}
      <span
        ref={useZoneDrag ? setZoneDragRef : undefined}
        {...(useZoneDrag ? { ...zoneDragAttrs, ...zoneDragListeners } : {})}
        onPointerDown={isPinned ? (e) => overlayDrag?.start(e) : undefined}
        className={cn(
      ...
      >
        {isDraggable && <GripVertical className="h-3 w-3 text-muted-foreground/40 shrink-0" />}
        {headerTitle}
      </span>
