...
export function SplitEditorPane({ externalDnd }: { externalDnd?: boolean }) {
  ...
  const showToolbarRow = showBreadcrumbs || hasModuleToolbar || hasModuleFilters;
  ...
  return (
    <EditorGroupProvider groupId="split">
      <div style={{ height: '100%', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        {showToolbarRow && (
          <div className="relative z-30 shrink-0 flex items-center gap-2 px-4 py-1.5 border-b border-border/40 glass-chrome min-h-8">
            {showBreadcrumbs && activePath && (
              <Breadcrumbs className="min-w-0" pathOverride={activePath} />
            )}
            <ModuleToolbar className={showBreadcrumbs ? 'ml-auto' : ''} />
          </div>
        )}
