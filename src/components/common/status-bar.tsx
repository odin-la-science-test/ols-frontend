...
import { Wifi, WifiOff, Bell, FlaskConical, CloudOff } from 'lucide-react';
...
// Presence desormais geree via SSE + online-users-store (green dots sur avatars)
...
export function StatusBar() {
  ...
  const { isOnline: queueOnline, pendingCount } = useOfflineStatus();
  ...
  return (
    <div data-tour="status-bar" className="hidden lg:flex items-center h-6 border-t border-border/30 surface-low text-[10px] select-none shrink-0" onContextMenu={handleContextMenu}>
      {/* ─── Left accent stripe (colored for modules, neutral for system) ─── */}
      <div
        className={cn('w-[2px] h-full shrink-0', !accentColor && 'system-indicator')}
        style={accentColor ? { backgroundColor: accentColor } : undefined}
      />

      {/* ─── Offline indicator (left side) ─── */}
      {!queueOnline && (
        <StatusBarSegment
          text={pendingCount > 0
            ? `${t('statusBar.offlineLabel')} (${t('statusBar.pending', { count: pendingCount })})`
            : t('statusBar.offlineLabel')}
          icon={undefined}
          tooltip={t('statusBar.offlineTooltip')}
          className="text-warning shrink-0"
          customIcon={<CloudOff className="h-2.5 w-2.5" />}
        />
      )}
      ...
      <div className="flex items-center gap-0.5 px-2 shrink-0">
        ...
        <StatusBarSegment
          ...
          className={IS_BETA ? 'text-foreground/60' : 'text-foreground/40'}
      ...
      {label && (
        <StatusBarSegment
          ...
          customIcon={
            splitMode ? (
              <span
                className={cn('inline-block w-1.5 h-1.5 rounded-full shrink-0', !accentColor && 'system-dot')}
                style={accentColor ? { backgroundColor: accentColor } : undefined}
              />
            ) : undefined
          }
...
function StatusBarSegment({ text, icon, tooltip, onClick, className, style, customIcon }: StatusBarSegmentProps) {
  ...
  const btn = (
    ...
    >
      {customIcon ?? (icon && <DynamicIcon name={icon} className="h-2.5 w-2.5" />)}
