...
import { classicTour } from '@/lib/tour/classic-tour';
...
export function TourLauncher() {
  ...
  useEffect(() => {
    ...
    // Workspace and global shell tours reference IDE elements — skip in classic mode
    if (!isClassic && location.pathname === '/workspace' && !isTourCompleted('workspace')) {
      const timer = setTimeout(() => runTour(workspaceTour), PAGE_TOUR_DELAY);
      return () => clearTimeout(timer);
    }

    // Module pages — shell tour takes priority on first module visit
    const mod = registry.getByRoute(location.pathname);
    if (!mod) return;

    if (!hasTriggeredGlobalShell.current) {
      const shellTourId = isClassic ? 'classic-shell' : 'global-shell';
      const shellTour = isClassic ? classicTour : globalTour;
      if (!isTourCompleted(shellTourId)) {
        hasTriggeredGlobalShell.current = true;
        const timer = setTimeout(() => runTour(shellTour), PAGE_TOUR_DELAY);
        return () => clearTimeout(timer);
      }
    }
