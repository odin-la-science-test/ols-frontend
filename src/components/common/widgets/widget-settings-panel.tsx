...
export function WidgetSettingsPanel({ widgetId, settings }: WidgetSettingsPanelProps) {
  ...
  return (
    <div className="space-y-2 p-2 rounded-lg bg-[color-mix(in_srgb,var(--color-card)_80%,transparent)] backdrop-blur-xl border border-[color-mix(in_srgb,var(--color-border)_50%,transparent)] shadow-2xl">
      {settings.map((def) => (
        <div key={def.key} className="flex items-center justify-between gap-3">
          ...
          {def.type === 'select' && (
            <div className="flex gap-0.5 bg-[color-mix(in_srgb,var(--color-muted)_40%,transparent)] rounded-md p-0.5">
              {def.options.map((opt) => (
                <button
                  key={opt.value}
                  ...
                  className={cn(
                    ...
                    resolved[def.key] === opt.value
                      ? 'bg-[color-mix(in_srgb,var(--color-primary)_20%,transparent)] text-primary'
