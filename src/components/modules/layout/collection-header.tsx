...
import { CheckSquare, Filter, GitCompareArrows, Search as SearchIcon, Trash2, X } from 'lucide-react';
...
interface SelectionModeBarProps {
  selection: Pick<UseSelectionReturn<{ id: string | number }>, 'isSelectionMode' | 'selectionCount' | 'clearSelection' | 'selectedIds'>;
  showCompare: boolean;
  onOpenComparison: () => void;
  onBatchDelete?: (ids: Set<string | number>) => void;
}

export function SelectionModeBar({ selection, showCompare, onOpenComparison, onBatchDelete }: SelectionModeBarProps) {
  const { t } = useTranslation();
  const Icon = showCompare ? GitCompareArrows : CheckSquare;
        ...
        <div className="min-w-0">
          <span className="text-sm font-medium text-foreground">
            {selection.selectionCount === 0
              ? t('modules.selection.selectItems')
              : t('modules.selection.selected', { count: selection.selectionCount })}
          </span>
          {showCompare && selection.selectionCount > 0 && selection.selectionCount < 2 && (
            <span className="text-xs text-muted-foreground ml-1 hidden sm:inline">
              ({t('modules.selection.selectMin')})
            </span>
          )}
        </div>
      </div>
      <div className="flex items-center gap-2">
        {selection.selectionCount > 0 && (
          <Button size="sm" variant="ghost" onClick={selection.clearSelection} className="h-8 px-2 sm:px-3 text-xs">
            ...
            <span className="hidden sm:inline">{t('modules.selection.deselectAll')}</span>
          </Button>
        )}
        {showCompare && (
          <Button
        ...
        {onBatchDelete && selection.selectionCount > 0 && (
          <Button
            size="sm"
            variant="destructive"
            onClick={() => {
              if (window.confirm(t('common.batchDeleteConfirm', { count: selection.selectionCount }))) {
                onBatchDelete(selection.selectedIds);
              }
            }}
            className="h-8 px-4 flex-1 sm:flex-none"
          >
