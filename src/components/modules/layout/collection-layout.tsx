...
import { createViewModeAction, createSelectAction, createExportAction, createNewItemAction } from '@/lib/module-action-builders';
...
export function CollectionLayout<T extends { id: number; confidenceScore?: number }>({
  ...
  const d = useDensity();
  const moduleIcon = <DynamicIcon name={iconName} className="h-8 w-8 text-muted-foreground/50" />;
  ...
  const [comparisonPanelOpen, setComparisonPanelOpen] = useState(() => (shell.savedState?.comparisonPanelOpen as boolean) ?? false);
  ...
  const restoredSelectionIds = useMemo(
    () => {
      const ids = shell.savedState?.selectionIds as (string | number)[] | undefined;
  ...
  const selection = useSelection<T>(items || [], {
    ...
    initialSelectedIds: restoredSelectionIds,
    initialIsSelectionMode: (shell.savedState?.isSelectionMode as boolean) ?? false,
  });
  shell.persistRef.current = { searchQuery, activeFilters, sort, selectedItemId: selectedItem?.id ?? undefined, formMode, currentPage: pagination.currentPage, selectionIds: [...selection.selectedIds], isSelectionMode: selection.isSelectionMode, comparisonPanelOpen };
  ...
  const handleSelect = (item: T) => { if (selection.isSelectionMode) return; if (onItemClick) { onItemClick(item); return; } if (selectedItem?.id === item.id && shell.detailOpen) { handleCloseDetail(); return; } setFormMode(null); setSelectedItem(item); shell.setDetailOpen(true); };
  ...
  }, [moduleKeyProp, newItemConfig, renderEditor, hasCardView, viewMode, showCompare, showExport, onBatchDelete, selection.isSelectionMode, sortedItems.length, toggleViewMode, selection.toggleSelectionMode, handleExport, handleOpenCreate, shell.registerToolbar]);
  // Listen for selection toggle from the command palette
  useModuleEvent('platform:toggleSelection', useCallback(() => {
    if (showCompare || !!onBatchDelete) {
      selection.toggleSelectionMode();
    }
  }, [showCompare, onBatchDelete, selection.toggleSelectionMode]));
  ...
  return (
    <ModuleLayout>
      <ModuleHeader title={title} iconName={iconName} backTo={backTo} mobileMenuItems={mobileMenuItems} showFilters={hasSidebarSections || hasSearch} />
      ...
      <div className="relative flex flex-1 overflow-hidden">
        ...
        <ModuleContent data-tour="module-content">
          <div className={cn('flex-1 overflow-y-auto', d.contentPadding, d.contentGap)}>
            ...
            <SelectionModeBar selection={selection} showCompare={showCompare} onOpenComparison={() => { if (selection.selectionCount >= 2) setComparisonPanelOpen(true); }} onBatchDelete={handleBatchDelete} />
            ...
            {isLoading || isSearching ? (
              ...
              <EmptyState
                icon={searchQuery ? SearchIcon : moduleIcon}
