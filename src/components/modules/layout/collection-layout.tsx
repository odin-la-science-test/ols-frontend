...
import { createViewModeAction, createCompareAction, createExportAction, createNewItemAction } from '@/lib/module-action-builders';
import { logger } from '@/lib/logger';
import { getModuleIcon } from '@/lib/module-icons';
import { useViewStore } from '@/stores';
import { useSelection, toast, useDensity, useModuleShell } from '@/hooks';
...
export function CollectionLayout<T extends { id: number; confidenceScore?: number }>({
  ...
  shell.persistRef.current = { searchQuery, activeFilters, sort, selectedItemId: selectedItem?.id ?? undefined, formMode };
  ...
  const statusBarItems: StatusBarItem[] = useMemo(() => {
    ...
    return sbi;
  }, [statusPrefix, shell.editorGroupId, allItems?.length, filteredItems.length, selection.selectionCount, viewMode, hasCardView, sort, t]);
  ...
  }, [moduleKeyProp, newItemConfig, renderEditor, hasCardView, viewMode, showCompare, cardConfig, showExport, selection.isSelectionMode, sortedItems.length, toggleViewMode, selection.toggleSelectionMode, handleExport, handleOpenCreate, shell.registerToolbar]);
      ...
      <div className="relative flex flex-1 overflow-hidden">
        ...
        <ModuleContent>
          <div className={cn('flex-1 overflow-y-auto', d.contentPadding, d.contentGap)}>
            ...
            <SelectionModeBar selection={selection} onOpenComparison={() => { if (selection.selectionCount >= 2) setComparisonPanelOpen(true); }} />
            ...
            {isLoading || isSearching ? (
              ...
              <EmptyState title={translations.error} description={translations.errorDesc} action={refetch && <Button onClick={() => refetch()} variant="outline">{t('common.retry')}</Button>} />
              ...
              <DataCardGrid data={sortedItems} config={cardConfig} selectedId={selectedItem?.id} onSelect={handleSelect} isSelectionMode={selection.isSelectionMode} selectedIds={selection.selectedIds} onToggleSelection={selection.toggleSelection} isMaxSelectionReached={selection.isMaxReached} />
            ) : (
              <DataTable data={sortedItems} columns={columns} sort={sort} onSort={setSort} selectedId={selectedItem?.id} onSelect={handleSelect} isSelectionMode={selection.isSelectionMode} selectedIds={selection.selectedIds} onToggleSelection={selection.toggleSelection} isMaxSelectionReached={selection.isMaxReached} />
            )}
          </div>
        </ModuleContent>
