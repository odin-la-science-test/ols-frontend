...
import { useModulesByType, type AppModuleDTO } from '@/hooks';
...
function ModuleIcon({ name, className }: { name: string; className?: string }) {
  const IconComponent = getModuleIcon(name) as LucideIcon;
...
function DashboardModuleCard({ module, accentColor, delay = 0, onLockedClick }: ModuleCardProps) {
  ...
  return (
    <FeatureCard
      ...
      icon={<ModuleIcon name={module.icon} className="w-full h-full" />}
...
export function DashboardPageTemplate({
  ...
  const { data: modules, isLoading, isError, refetch } = useModulesByType(category);
  ...
  return (
    <div className="min-h-screen flex flex-col relative overflow-hidden">
      ...
      <AppTopBar maxWidth="max-w-7xl" />

      {/* Main Content */}
      <main className="relative z-10 flex-1 px-4 sm:px-6 md:px-8 py-6 sm:py-8 pb-24 lg:pb-8">
