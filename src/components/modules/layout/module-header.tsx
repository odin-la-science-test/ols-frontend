...
interface ModuleHeaderProps {
  ...
  title: string;
  /** Lucide icon name (kebab-case) — shown only on mobile */
  iconName?: string;
      ...
      <div className={cn(
        ...
        <div className="flex items-center gap-1 min-w-0">
          ...
          <div className="flex items-center gap-2 mr-2 min-w-0">
            {iconName && <DynamicIcon name={iconName} className="h-4 w-4 text-foreground shrink-0" strokeWidth={1.5} />}
                  ...
                  return (
                    <span key={action.id}>
                      ...
                      <DropdownMenuItem onClick={action.action} disabled={isDisabled}>
                        <DynamicIcon name={action.icon} className={cn('h-4 w-4 mr-2', isActive ? 'text-primary' : '')} />
