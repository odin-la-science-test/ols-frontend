...
import { FlaskConical, Microscope, Star } from 'lucide-react';
...
export const getGramLabels = (t: TFunction): Record<GramStatus, string> => ({
  ...
  VARIABLE: t('scientific.gramVariable'),
});
...
const getBaseColumns = (t: TFunction): ColumnDef<Bacterium>[] => {
      ...
      <div className="font-medium flex items-center min-w-0">
        <Tooltip delayDuration={200}>
          <TooltipTrigger asChild>
            <span className="italic truncate">{value as string}</span>
          </TooltipTrigger>
          <TooltipContent side="top">{value as string}</TooltipContent>
        </Tooltip>
        {row.strain && (
          <Tooltip delayDuration={200}>
            <TooltipTrigger asChild>
              <span className="ml-2 text-muted-foreground text-xs truncate shrink-0 max-w-[150px]">
                {row.strain}
              </span>
            </TooltipTrigger>
            <TooltipContent side="top">{row.strain}</TooltipContent>
          </Tooltip>
        )}
      </div>
    ),
  ...
  },
  {
    ...
    render: (value) => {
      ...
      return (
        <Badge variant={gram === 'POSITIVE' ? 'gramPositive' : gram === 'NEGATIVE' ? 'gramNegative' : 'outline'} size="sm">
          {gramLabels[gram]}
        </Badge>
      );
    },
...
export const getBacteriaFilters = (t: TFunction): FilterConfig[] => [
  ...
  },
  {
    ...
    options: [
      ...
      { value: 'VARIABLE', label: t('scientific.gramVariable') },
    ],
  },
  {
    key: 'morphology',
