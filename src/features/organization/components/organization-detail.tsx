    ...
    <DetailPanelContent title={organization.name} icon={Building2}>
      ...
      <DetailSection title={`${t('organization.supervisions')} (${supervisions?.length ?? 0})`} icon={GitBranch}>
        {supervisions && supervisions.length > 0 ? (
          <div className="space-y-1.5">
            {supervisions.map((s) => (
              <div key={s.id} className="flex items-center gap-1.5 text-sm py-1">
                <UserAvatar userId={s.supervisorId} avatarId={s.supervisorAvatarId} name={s.supervisorName} size="xs" />
                <span className="font-medium">{s.supervisorName}</span>
                <span className="text-muted-foreground mx-1">→</span>
                <UserAvatar userId={s.superviseeId} avatarId={s.superviseeAvatarId} name={s.superviseeName} size="xs" />
