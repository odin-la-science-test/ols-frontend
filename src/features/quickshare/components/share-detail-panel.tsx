...
import { Copy, Check, Download, FileText, File, Clock, Eye, X, ArrowLeft, Archive, UserCheck } from 'lucide-react';
...
export function ShareDetailPanel({ item, onClose }: ShareDetailPanelProps) {
  ...
  const copyTextContent = async () => {
    if (item.textContent) {
      ...
      toast({ title: t('quickshare.textCopied') });
    }
  };

  const handleDownload = async (fileId: number, filename: string) => {
    try {
      const response = await quickShareApi.downloadFile(item.shareCode, fileId);
  ...
  const handleDownloadAll = async () => {
    try {
      const response = await quickShareApi.downloadAll(item.shareCode);
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 p-4 border-b border-border/30">
        ...
        >
          {isText ? <FileText className="w-4 h-4" strokeWidth={1.5} /> : <File className="w-4 h-4" strokeWidth={1.5} />}
        </div>
