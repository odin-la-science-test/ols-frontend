...
export interface UseSelectionOptions {
  ...
  onMaxReached?: () => void;
  /** Initial selected IDs (for restoring persisted state) */
  initialSelectedIds?: Set<string | number>;
  /** Initial selection mode state (for restoring persisted state) */
  initialIsSelectionMode?: boolean;
}
...
export function useSelection<T extends { id: string | number }>(
  ...
  const { maxSelection = 10, onSelectionChange, onMaxReached, initialSelectedIds, initialIsSelectionMode } = options;

  const [selectedIds, setSelectedIds] = useState<Set<string | number>>(() => initialSelectedIds ?? new Set());
  const [isSelectionMode, setIsSelectionMode] = useState(() => initialIsSelectionMode ?? false);
