import type { ModuleDefinition, ModuleSearchProvider, ModuleActivityPanel, ModuleSettingsSection, TourStep } from './types';
...
class ModuleRegistry {
  ...
  getSettings(): Array<{ module: ModuleDefinition; sections: ModuleSettingsSection[] }> {
    return this.getAll()
      .filter((m): m is ModuleDefinition & { settings: ModuleSettingsSection[] } =>
        !!m.settings && m.settings.length > 0
      )
      .map((m) => ({ module: m, sections: m.settings }));
  }

  /** Get all contextual tips from all modules */
  getTips(): Array<{ module: ModuleDefinition; tips: ContextualTip[] }> {
    return this.getAll()
      .filter((m): m is ModuleDefinition & { tips: ContextualTip[] } =>
        !!m.tips && m.tips.length > 0
      )
      .map((m) => ({ module: m, tips: m.tips }));
  }
...
/** Filter modules to only those the user can access (use with module-access-store) */
export function filterAccessibleModules(
  modules: ModuleDefinition[],
  canAccess: (key: string) => boolean,
): ModuleDefinition[] {
  return modules.filter((m) => canAccess(m.moduleKey));
}
